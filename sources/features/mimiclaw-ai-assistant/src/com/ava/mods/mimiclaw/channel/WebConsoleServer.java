...
public class WebConsoleServer {
    ...
    private String buildPage(boolean authed, String sessionId) {
        ...
        return "<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no,viewport-fit=cover'>"
            ...
            + "function globalMicRelease(e){if(!micPressActive&&!micLongPress&&!micPressTimer)return;if(micPressTimer){clearTimeout(micPressTimer);micPressTimer=null;micPressActive=false;toggleVoiceRecording();return;}if(micLongPress&&window.__voiceRecording){stopVoiceRecording(true);}micPressActive=false;micLongPress=false;}"
