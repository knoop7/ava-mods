...
public class WebConsoleServer {
    ...
    private ParsedRequest readRequest(InputStream input) throws Exception {
        ...
        if (contentLength > 0) {
            ...
            request.body = new String(bodyBytes, 0, read, StandardCharsets.UTF_8);
        } else {
            request.body = "";
        }
        return request;
    ...
    private String buildPage(boolean authed, String sessionId) {
        ...
        return "<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no,viewport-fit=cover'>"
            ...
            + "function appendBubbleNode(cls,html){if(!html)return;const el=document.createElement('div');el.className='bubble '+cls;el.innerHTML=(cls==='user')?escapeHtml(html):html;document.getElementById('messages').appendChild(el);}"
            ...
            + "function renderUserAttachments(files){if(!files||files.length===0)return'';let h='<div class=\"userAttachments\">';files.forEach(f=>{const isImg=f.type&&f.type.startsWith('image/');if(isImg){const url=URL.createObjectURL(f);h+='<div class=\"userAttachItem\"><img src=\"'+url+'\" onclick=\"window.open(this.src)\"/></div>';}else{h+='<div class=\"userAttachItem file\"><span>'+escapeHtml(f.name)+'</span></div>';}});h+='</div>';return h;}"
            ...
            + "(async function init(){window.__openclawBusy=false;setComposerBusy(false);const cb=document.getElementById('showToolCalls');if(cb)cb.checked=showToolCalls;maybeShowInstallHint();await loadConfig();await loadSkillControl();await reloadHistory();applyToolCallVisibility();setTimeout(connectSSE,100);})();"
            + "</script></body></html>";
    }
