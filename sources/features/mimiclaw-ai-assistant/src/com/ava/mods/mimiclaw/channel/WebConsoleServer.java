...
public class WebConsoleServer {
    ...
    private String buildPage(boolean authed, String sessionId) {
        ...
        return "<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no,viewport-fit=cover'>"
            ...
            + "function extractMediaFromToolResult(c){let media='';let rest=c;try{const j=JSON.parse(c);if(j.user_image){media+='<div class=\"imgWrap\"><img src=\"'+fileUrl(j.user_image)+'\"/></div>';rest=j.message||'';}}catch(e){const imgMatch=rest.match(/<media-img>([^<]+)<\\/media-img>/);if(imgMatch){media+='<div class=\"imgWrap\"><img src=\"'+fileUrl(imgMatch[1].trim())+'\"/></div>';rest=rest.replace(imgMatch[0],'');}rest=rest.replace(/<image_data>[^<]+<\\/image_data>/g,'').trim();}return {media,rest};}"
