...
# Ava Mod Build Script - Camera Stream (MJPEG / RTSP)
...
BUILD_DIR="build"
OUTPUT_JAR="libs/camera-stream-manager.jar"
RELEASE_DIR="../../../mods/features/camera-stream-mod"
...
javac -source 1.8 -target 1.8 \
    -cp "$ANDROID_JAR" \
    -d "$BUILD_DIR" \
    src/com/ava/mods/camerastream/*.java
...
"$D8_TOOL" --output "$BUILD_DIR" "$BUILD_DIR"/com/ava/mods/camerastream/*.class
...
echo "Syncing release package to $RELEASE_DIR ..."
mkdir -p "$RELEASE_DIR/libs"
cp -f "$OUTPUT_JAR" "$RELEASE_DIR/libs/"
cp -f manifest.json "$RELEASE_DIR/manifest.json"

echo "Done! JAR at $OUTPUT_JAR"
ls -la "$OUTPUT_JAR"
unzip -l "$OUTPUT_JAR"
