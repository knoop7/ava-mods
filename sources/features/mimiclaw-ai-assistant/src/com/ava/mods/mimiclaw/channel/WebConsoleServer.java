...
public class WebConsoleServer {
    ...
    private void handleClient(Socket socket) {
        try (Socket client = socket) {
            ...
            if ("GET".equals(method) && path.startsWith("/api/file")) {
                ...
                byte[] data = readFileAsBytes(file);
    ...
    private String buildPage(boolean authed, String sessionId) {
        ...
        return "<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no,viewport-fit=cover'>"
            ...
            + ".cardStatus{margin-left:auto;font-size:10px;line-height:1;color:var(--accent);font-weight:700;letter-spacing:.02em;max-width:80px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}"
            ...
            + ".metaRow{display:flex;align-items:center;gap:8px;padding:6px 12px 0 28px;}.statusDot{width:6px;height:6px;background:#999;flex-shrink:0;}.statusDot.st-ai{background:#a855f7;}.statusDot.st-ok{background:#22c55e;}.statusDot.st-warn{background:#eab308;}.statusDot.st-err{background:#ef4444;}.statusDot.spin{animation:pulse .8s ease-in-out infinite;}@keyframes pulse{0%,100%{opacity:1;}50%{opacity:.3;}}"
            ...
            + ".workspaceStatValue{font-size:12px;line-height:1.35;color:var(--text);font-weight:600;white-space:nowrap;overflow-x:auto;overflow-y:hidden;max-height:1.35em;}"
            + ".workspaceJson{margin-top:0;max-height:140px;font-size:10px;line-height:1.5;padding:10px 11px;border-radius:12px;}"
            + ".composerWrap{padding:8px 16px;background:var(--panel);}"
            ...
            + ".meta{padding:0;color:var(--muted);font-size:12px;line-height:1;}"
            ...
            + "function extractMediaFromToolResult(c){let media='';let rest=c;try{const j=JSON.parse(c);if(j.user_image){rest=j.message||'';}}catch(e){rest=rest.replace(/<media-img>[^<]+<\\/media-img>/g,'');rest=rest.replace(/<image_data>[^<]+<\\/image_data>/g,'').trim();}return {media,rest};}"
            + "function renderToolBlocks(blocks){let h='';(blocks||[]).forEach(t=>{try{if(t.type==='tool_use'){const id='tc_'+(++toolCallId);const name=escapeHtml(String(t.name||'tool'));let inputStr='';try{inputStr=JSON.stringify(t.input||{},null,2);}catch(e){inputStr=String(t.input||'{}');}h+='<details class=\"toolCall\" id=\"'+id+'\"><summary onclick=\"event.stopPropagation()\">'+svgBolt+' '+name+'</summary><pre><code>'+escapeHtml(truncate(inputStr,4000))+'</code></pre></details>';}if(t.type==='tool_result'){const id='tc_'+(++toolCallId);let c='';try{c=typeof t.content==='string'?t.content:JSON.stringify(t.content,null,2);}catch(e){c=String(t.content||'');}const extracted=extractMediaFromToolResult(c);if(extracted.media){h+=extracted.media;}if(extracted.rest){h+='<details class=\"toolCall\" id=\"'+id+'\"><summary onclick=\"event.stopPropagation()\">'+svgBolt+' Result</summary><pre><code>'+escapeHtml(truncate(extracted.rest,4000))+'</code></pre></details>';}}}catch(e){console.error('renderToolBlocks error',e);}});return h;}"
            ...
            + "function renderPlainContent(text){if(!text)return'';if(looksLikeToolJson(text))return'';let html=text;html=html.replace(/\\[TASK_TREE_EMPTY\\]/g,'<div class=\"taskTreeEmpty\"><div class=\"taskTreeTitle\">📋 Task Tree</div><div class=\"taskTreeHint\">No tasks. Use task_create tool to add tasks.</div></div>');html=html.replace(/\\[TASK_TREE:(\\{[^\\]]+\\})\\]/g,function(m,json){try{const data=JSON.parse(json);return renderTaskTreeHtml(data);}catch(e){return m;}});html=html.replace(/<image_data>([^<]+)<\\/image_data>/g,(m,d)=>{let url=d.trim();if(!url.startsWith('data:'))url='data:'+url;return '<div class=\"imgWrap\"><img src=\"'+url+'\"/></div>';});html=html.replace(/\\[IMAGE:[^\\]]*\\]\\n?/g,'');html=html.replace(/<media-img>([^<]+)<\\/media-img>/g,(m,p)=>'<div class=\"imgWrap\"><img src=\"'+fileUrl(p.trim())+'\"/></div>');html=html.replace(/<media-audio>([^<]+)<\\/media-audio>/g,(m,p)=>'<audio controls src=\"'+fileUrl(p.trim())+'\" style=\"width:100%;margin:8px 0;\"></audio>');html=html.replace(/<media-file name=\"([^\"]+)\">([^<]+)<\\/media-file>/g,(m,n,p)=>'<a href=\"'+fileUrl(p.trim())+'\" target=\"_blank\" style=\"display:inline-flex;align-items:center;gap:6px;padding:8px 12px;background:var(--field);border:1px solid var(--line);border-radius:8px;text-decoration:none;color:var(--text);margin:4px 0;\">'+n+'</a>');html=html.replace(/\\[image:([^\\]]+)\\]/g,(m,p)=>'<div class=\"imgWrap\"><img src=\"'+fileUrl(p)+'\"/></div>');html=html.replace(/\\[audio:([^\\]]+)\\]/g,(m,p)=>'<audio controls src=\"'+fileUrl(p)+'\" style=\"width:100%;margin:8px 0;\"></audio>');html=html.replace(/\\[file:([^|]+)\\|([^\\]]+)\\]/g,(m,p,n)=>'<a href=\"'+fileUrl(p)+'\" target=\"_blank\" style=\"display:inline-flex;align-items:center;gap:6px;padding:8px 12px;background:var(--field);border:1px solid var(--line);border-radius:8px;text-decoration:none;color:var(--text);margin:4px 0;\">'+n+'</a>');if(typeof marked!=='undefined'){try{html=marked.parse(html).trim();}catch(e){html=escapeHtml(text).replace(/\\n/g,'<br>');}}else{html=escapeHtml(text).replace(/\\n/g,'<br>');}return decorateCodeBlocks(html.trim());}"
            + "function renderTaskTreeHtml(data){const total=data.total||0;const pending=data.pending||0;const inProgress=data.in_progress||0;const completed=data.completed||0;const failed=data.failed||0;const todos=data.todos||[];let h='<div class=\"taskTree\"><div class=\"taskTreeHeader\"><div class=\"taskTreeTitle\">📋 Task Tree</div><div class=\"taskTreeStats\">'+completed+'/'+total+' done</div></div>';h+='<div class=\"taskTreeSummary\"><span class=\"taskTreeStat pending\">⏳ '+pending+'</span><span class=\"taskTreeStat progress\">🔄 '+inProgress+'</span><span class=\"taskTreeStat done\">✅ '+completed+'</span>'+(failed>0?'<span class=\"taskTreeStat fail\">❌ '+failed+'</span>':'')+'</div>';if(todos.length>0){h+='<div class=\"taskTreeList\">';todos.forEach((t,i)=>{const st=t.status||'pending';const icon=st==='completed'?'✅':st==='in_progress'?'🔄':st==='failed'?'❌':'⏳';const content=escapeHtml(t.content||t.activeForm||'Task '+(i+1));h+='<div class=\"taskTreeItem '+st+'\"><span class=\"taskTreeIcon\">'+icon+'</span><span class=\"taskTreeContent\">'+content+'</span><span class=\"taskTreeBadge\">'+st+'</span></div>';});h+='</div>';}h+='</div>';return h;}"
            ...
            + "function isTimerMsg(c){return c&&(c.includes('[timer]')||c.startsWith('HEARTBEAT_TICK:')||c.startsWith('CRON_TICK:'));}"
            + "function renderHistory(items){const box=document.getElementById('messages');box.innerHTML='';shownMsgs.clear();(items||[]).filter(item=>!isTimerMsg(item.content)).forEach(item=>addBubble(item.content||'',bubbleClass(item.role||'assistant'),false,item.timestamp||''));box.scrollTop=1e9;highlightCodeBlocks(box);initViewer();applyToolCallVisibility();}"
            ...
            + "function parseErrorCode(s){if(!s)return null;const m=s.match(/(\\d{3})/);return m?m[1]:null;}function renderWorkspace(status,detail){detail=detail||{};const tokens=document.getElementById('wsTokens');const model=document.getElementById('wsModel');const history=document.getElementById('wsHistory');const err=document.getElementById('wsError');const statusEl=document.getElementById('workspaceStatus');const normalizedStatus=humanStatus(detail.agentStatus||status);if(tokens)tokens.textContent=workspaceValue(detail.totalTokens,'0');if(model)model.textContent=workspaceValue(detail.model,'-');if(history)history.textContent=workspaceValue(detail.historyCount,'0');const errStr=detail.webConsoleError||'';const code=parseErrorCode(errStr)||parseErrorCode(normalizedStatus);const isErr=normalizedStatus&&String(normalizedStatus).indexOf('error')>=0;if(err)err.textContent=code?code:(isErr?'Err':'None');if(statusEl)statusEl.textContent=isErr?(code||'Err'):workspaceValue(normalizedStatus,'idle');renderStatusJson(detail);}"
            ...
            + "function setChatStatus(status,opts){const el=document.getElementById('chatStatus');if(!el)return;const options=opts||{};if(options.plain){const spin=options.className&&(options.className.includes('st-ai')||options.className.includes('st-warn'))?' spin':'';el.className='statusDot'+(options.className?' '+options.className:'')+spin;return;}updateStatusIndicator(status);}"
            ...
            + "function startNewChat(){document.getElementById('message').value='/new';sendChat();}"
            ...
            + "function displayCronName(job){if(!job)return'Task';if(job.title)return job.title;const raw=job.name||'Task';return raw==='__builtin_heartbeat__'?'Heartbeat':raw;}"
            + "function formatCronMeta(job){if(!job)return'';if(job.description)return job.description;const isHeartbeat=job&&job.name==='__builtin_heartbeat__';const seconds=Number(job.interval_s||0);const interval=seconds>=60?Math.round(seconds/60)+'m':seconds+'s';if(isHeartbeat)return 'Every '+interval;if(job.kind==='every')return 'Every '+interval;if(job.kind==='at'){const at=Number(job.at_epoch||0);if(at>0){try{const d=new Date(at*1000);return d.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'});}catch(e){}}return 'Once';}return '';}"
            ...
            + "function renderCronList(cronJobs){const list=document.getElementById('cronList');if(!list)return;let h='';if(Array.isArray(cronJobs)&&cronJobs.length>0){h+='<div class=\"cronList\">';cronJobs.forEach(job=>{const name=displayCronName(job);const meta=formatCronMeta(job);const checked=job.enabled===true;const id=job&&job.id?job.id:'';const badge=checked?'On':'Off';h+='<label class=\"skillItem cronItem\"><input type=\"checkbox\" data-job-id=\"'+escapeHtml(id)+'\" '+(checked?'checked':'')+' onchange=\"toggleCron(this)\"><div class=\"cronBody\"><div class=\"cronNameRow\"><div class=\"cronName\">'+escapeHtml(name)+'</div><div class=\"cronBadge '+(checked?'on':'off')+'\">'+badge+'</div></div><div class=\"cronMeta\">'+escapeHtml(meta)+'</div></div></label>';});h+='</div>';}else{h='<div class=\"cronList\"><div class=\"cronItem\"><div class=\"cronBody\"><div class=\"cronNameRow\"><div class=\"cronName\">No scheduled tasks</div></div><div class=\"cronMeta\">Timers created by AI will appear here.</div></div></div></div>';}list.innerHTML=h;}"
            ...
            + "let lastHistoryCount=0;async function syncHistory(){try{const r=await fetch(apiUrl('/api/history'),{credentials:'include',headers:apiHeaders()});const j=await r.json();if(j.ok){const msgs=(j.messages||[]).filter(m=>!isTimerMsg(m.content));if(msgs.length!==lastHistoryCount){lastHistoryCount=msgs.length;renderHistory(msgs);}}}catch(e){}}setInterval(syncHistory,5000);"
    ...
    private String baseCss() {
        return ":root{color-scheme:light;--font:'SF Pro Display','Segoe UI',system-ui,sans-serif;--bg:#faf8ff;--sidebar:#f3f3fc;--panel:#ffffff;--panelAlt:#ededf6;--field:#f3f3fc;--line:#e2e8f0;--text:#191b22;--muted:#64748b;--accent:#0417e0;--accentSoft:#eef2ff;--accentText:#ffffff;--userBubble:#0417e0;--userText:#ffffff;--botBubble:#e2e8f0;--attachBg:#ffffff;--codeBg:#f6f8fc;--codeTop:#edf2f8;--codeLine:#d8e0ec;--codeText:#1f2937;--codeMuted:#7c8798;--shadow:0 22px 60px rgba(24,39,75,.10);--userShadow:0 8px 24px rgba(37,99,235,.18);--scrollTrack:#f3f3fc;--scrollThumb:#c4c9d4;}html[data-theme='dark']{color-scheme:dark;--bg:#111319;--sidebar:#191b22;--panel:#1e1f26;--panelAlt:#282a30;--field:#2d2d2d;--line:#3d3d3d;--text:#f1f5f9;--muted:#94a3b8;--accent:#a78b73;--accentSoft:#3a322c;--accentText:#fffaf6;--userBubble:#a78b73;--userText:#ffffff;--botBubble:#2d2d2d;--attachBg:#1e1b1b;--codeBg:#161b22;--codeTop:#1f2630;--codeLine:#2d3641;--codeText:#e5edf6;--codeMuted:#8ea0b5;--shadow:0 22px 60px rgba(0,0,0,.34);--userShadow:0 8px 24px rgba(167,139,115,.25);--scrollTrack:#1e1f26;--scrollThumb:#4a4d55;}*{box-sizing:border-box;}button,input{font:inherit;}::-webkit-scrollbar{width:8px;height:8px;}::-webkit-scrollbar-track{background:var(--scrollTrack);border-radius:4px;}::-webkit-scrollbar-thumb{background:var(--scrollThumb);border-radius:4px;}::-webkit-scrollbar-thumb:hover{background:var(--muted);}.taskTree{margin:10px 0;padding:12px;border:1px solid var(--line);border-radius:12px;background:var(--panelAlt);}.taskTreeHeader{display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;}.taskTreeTitle{font-size:14px;font-weight:600;color:var(--text);}.taskTreeStats{font-size:12px;color:var(--muted);}.taskTreeSummary{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px;}.taskTreeStat{font-size:11px;padding:3px 8px;border-radius:6px;background:var(--field);}.taskTreeStat.pending{color:#eab308;}.taskTreeStat.progress{color:#3b82f6;}.taskTreeStat.done{color:#22c55e;}.taskTreeStat.fail{color:#ef4444;}.taskTreeList{display:flex;flex-direction:column;gap:4px;}.taskTreeItem{display:flex;align-items:flex-start;gap:6px;padding:8px 10px;background:var(--field);border-radius:8px;font-size:13px;}.taskTreeItem.completed{opacity:0.7;}.taskTreeItem.in_progress{border-left:3px solid var(--accent);}.taskTreeItem.failed{border-left:3px solid #ef4444;}.taskTreeIcon{flex-shrink:0;}.taskTreeContent{flex:1;min-width:0;word-break:break-word;color:var(--text);}.taskTreeBadge{font-size:10px;padding:2px 6px;border-radius:4px;background:var(--panel);color:var(--muted);flex-shrink:0;}.taskTreeEmpty{padding:12px;border:1px dashed var(--line);border-radius:12px;text-align:center;}.taskTreeHint{font-size:12px;color:var(--muted);}";
    }

    private byte[] readFileAsBytes(java.io.File file) throws Exception {
        // Android 8.0+ (API 26) 支持 java.nio.file.Files
        if (android.os.Build.VERSION.SDK_INT >= 26) {
            return java.nio.file.Files.readAllBytes(file.toPath());
        }
        // Android 7.x 及以下使用 FileInputStream
