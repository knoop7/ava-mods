...
public class WebConsoleServer {
    ...
    private String buildPage(boolean authed, String sessionId) {
        ...
        return "<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no,viewport-fit=cover'>"
            ...
            + ".sidebar{width:350px;flex-shrink:0;background:var(--sidebar);border-right:1px solid var(--line);padding:calc(24px + env(safe-area-inset-top,0px)) 18px calc(24px + env(safe-area-inset-bottom,0px));display:flex;flex-direction:column;gap:18px;overflow-y:auto;}"
            ...
            + ".cronItem{display:flex;align-items:flex-start;gap:8px;padding:6px 0;font-size:12px;color:var(--text);}"
            + ".cronItem svg{width:14px;height:14px;flex-shrink:0;color:var(--accent);margin-top:1px;}"
            + ".cronBody{display:flex;flex-direction:column;gap:2px;min-width:0;}"
            + ".cronName{font-size:12px;font-weight:600;color:var(--text);}"
            + ".cronMeta{font-size:11px;line-height:1.4;color:var(--muted);word-break:break-word;}"
            ...
            + "<div class='settingsCard'><div class='navTitle'>Scheduled</div><details open><summary>" + svgHeart + " Timers</summary><div class='settingsRow compact' id='cronList'></div></details></div>"
            ...
            + "function renderHistory(items){const box=document.getElementById('messages');box.innerHTML='';shownMsgs.clear();(items||[]).forEach(item=>addBubble(item.content||'',bubbleClass(item.role||'assistant'),false,item.timestamp||''));box.scrollTop=1e9;initViewer();applyToolCallVisibility();}"
            ...
            + "async function renameCurrentProfile(){if(profileRenameTimer)clearTimeout(profileRenameTimer);profileRenameTimer=setTimeout(async()=>{const nameInput=document.getElementById('cfgProfileName');const profileName=nameInput&&nameInput.value?nameInput.value.trim():'';if(!activeProfileId||!profileName)return;try{const r=await fetch(apiUrl('/api/config/profile'),{method:'POST',credentials:'include',headers:apiHeaders({'Content-Type':'application/json'}),body:JSON.stringify({action:'rename',profile_id:activeProfileId,profile_name:profileName})});const j=await r.json();if(j.ok){await loadConfig();document.getElementById('configStatus').textContent='Saved';}else{document.getElementById('configStatus').textContent='Failed';}}catch(e){document.getElementById('configStatus').textContent='Error';}},350);}"
            + "async function createProfileDraft(){try{const r=await fetch(apiUrl('/api/config/profile'),{method:'POST',credentials:'include',headers:apiHeaders({'Content-Type':'application/json'}),body:JSON.stringify({action:'create',make_active:true})});const j=await r.json();if(j.ok){await loadConfig();document.getElementById('configStatus').textContent='New profile created';}else{document.getElementById('configStatus').textContent='Create failed';}}catch(e){document.getElementById('configStatus').textContent='Error';}}"
            + "async function deleteCurrentProfile(){if(!activeProfileId)return;const total=(providerProfiles||[]).length;if(total<=1){document.getElementById('configStatus').textContent='Keep at least one profile';return;}const active=(providerProfiles||[]).find(p=>p&&p.id===activeProfileId);const name=active&&active.name?active.name:activeProfileId;if(!confirm('Delete profile \"'+name+'\"?'))return;try{const r=await fetch(apiUrl('/api/config/profile'),{method:'POST',credentials:'include',headers:apiHeaders({'Content-Type':'application/json'}),body:JSON.stringify({action:'delete',profile_id:activeProfileId})});const j=await r.json();if(j.ok){await loadConfig();document.getElementById('configStatus').textContent='Profile deleted';}else{const reason=j&&j.result&&j.result.reason?j.result.reason:'';document.getElementById('configStatus').textContent=reason==='last_profile'?'Keep at least one profile':'Delete failed';}}catch(e){document.getElementById('configStatus').textContent='Error';}}"
            ...
            + "function formatCronMeta(job){if(!job)return'';if(job.kind==='every'){return 'Every '+String(job.interval_s||0)+'s';}if(job.kind==='at'){return 'At '+String(job.at_epoch||0);}return '';}"
            + "async function toggleCron(cb){const jobId=cb.dataset.jobId||'';if(!jobId)return;try{const r=await fetch(apiUrl('/api/skill_config'),{method:'POST',credentials:'include',headers:apiHeaders({'Content-Type':'application/json'}),body:JSON.stringify({cron_job_id:jobId,enabled:!!cb.checked})});const j=await r.json();document.getElementById('skillStatus').textContent=j.ok?'Saved':'Failed';setTimeout(()=>{document.getElementById('skillStatus').textContent='';},1500);if(j.ok){await loadSkillControl();}}catch(e){document.getElementById('skillStatus').textContent='Error';}}"
            + "function renderCronList(cronJobs){const list=document.getElementById('cronList');if(!list)return;let h='';if(Array.isArray(cronJobs)&&cronJobs.length>0){h+='<div class=\"cronList\">';cronJobs.forEach(job=>{const name=job&&job.name?job.name:'Task';const meta=formatCronMeta(job);const checked=job&&job.enabled!==false;const id=job&&job.id?job.id:'';h+='<label class=\"skillItem cronItem\"><input type=\"checkbox\" data-job-id=\"'+escapeHtml(id)+'\" '+(checked?'checked':'')+' onchange=\"toggleCron(this)\"><span>'+svgHeart+'</span><div class=\"cronBody\"><div class=\"cronName\">'+escapeHtml(name)+'</div><div class=\"cronMeta\">'+escapeHtml(meta)+'</div></div></label>';});h+='</div>';}else{h='<div class=\"cronList\"><div class=\"cronItem\">'+svgHeart+'<div class=\"cronBody\"><div class=\"cronName\">No scheduled tasks</div><div class=\"cronMeta\">AI can manage timers with the existing cron tools.</div></div></div></div>';}list.innerHTML=h;}"
