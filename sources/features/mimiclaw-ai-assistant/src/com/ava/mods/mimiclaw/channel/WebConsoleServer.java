...
public class WebConsoleServer {
    ...
    private String buildPage(boolean authed, String sessionId) {
        ...
        return "<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no,viewport-fit=cover'>"
            ...
            + "<script src='https://cdn.jsdelivr.net/npm/highlight.js@11.10.0/lib/languages/java.min.js'></script>"
            ...
            + "let voiceRecognition=null;let voicePressTimer=null;let voicePointerActive=false;let voiceRecordingText='';let voicePermissionReady=false;let voicePressToken=0;let voiceTouchStartX=0;let voiceTouchStartY=0;const voiceTouchThreshold=12;window.__voiceRecording=false;window.__suppressSendClick=false;"
            ...
            + "async function startVoiceRecording(token){if(window.__openclawBusy||window.__voiceRecording||!voicePointerActive||token!==voicePressToken)return;const granted=await ensureMicPermission();if(!granted||!voicePointerActive||token!==voicePressToken){if(!granted)document.getElementById('chatStatus').textContent='Mic denied';return;}const recognition=ensureVoiceRecognition();if(!recognition){document.getElementById('chatStatus').textContent='Voice unsupported';return;}const btn=document.getElementById('sendBtn');const input=document.getElementById('message');voiceRecordingText=(input&&input.value?input.value.trim():'');window.__voiceRecording=true;window.__suppressSendClick=true;if(btn){btn.innerHTML=svgMicIcon;btn.title='Listening';btn.classList.add('recording');btn.classList.remove('busy');}if(input)input.placeholder='Listening...';document.getElementById('chatStatus').textContent='Listening...';try{recognition.start();}catch(e){window.__voiceRecording=false;if(btn){btn.innerHTML=svgSendIcon;btn.title='Send or hold to speak';btn.classList.remove('recording');}if(input)input.placeholder='Type a message...';document.getElementById('chatStatus').textContent='Voice unavailable';}}"
            + "function stopVoiceRecording(fromSystem){if(voicePressTimer){clearTimeout(voicePressTimer);voicePressTimer=null;}const wasRecording=window.__voiceRecording;window.__voiceRecording=false;voicePointerActive=false;const btn=document.getElementById('sendBtn');const input=document.getElementById('message');const shouldSend=!!(input&&input.value&&input.value.trim());if(voiceRecognition&&!fromSystem){try{voiceRecognition.stop();}catch(e){}}if(btn){btn.innerHTML=window.__openclawBusy?svgStopIcon:svgSendIcon;btn.title=window.__openclawBusy?'Stop':'Send or hold to speak';btn.classList.remove('recording');btn.classList.toggle('busy',!!window.__openclawBusy);}if(input)input.placeholder=window.__openclawBusy?'AI is processing...':'Type a message...';if(wasRecording){document.getElementById('chatStatus').textContent=shouldSend?'Sending voice...':'Ready';if(shouldSend&&!window.__openclawBusy&&!sending){setTimeout(()=>sendChat(),80);}setTimeout(()=>loadStatus(),220);}}"
            + "function scheduleVoiceStart(event){if(window.__openclawBusy)return;if(event&&event.type==='mousedown'&&event.button!==0)return;voicePointerActive=true;voicePressToken++;if(event&&event.touches&&event.touches[0]){voiceTouchStartX=event.touches[0].clientX;voiceTouchStartY=event.touches[0].clientY;}if(voicePressTimer)clearTimeout(voicePressTimer);const token=voicePressToken;voicePressTimer=setTimeout(()=>{voicePressTimer=null;if(voicePointerActive&&token===voicePressToken)startVoiceRecording(token);},420);}"
            + "function cancelVoicePress(){voicePointerActive=false;voicePressToken++;if(voicePressTimer){clearTimeout(voicePressTimer);voicePressTimer=null;}if(window.__voiceRecording)stopVoiceRecording(false);}"
            ...
            + "function setupVoiceInput(){const btn=document.getElementById('sendBtn');if(!btn)return;btn.addEventListener('mousedown',scheduleVoiceStart);btn.addEventListener('touchstart',function(event){event.preventDefault();scheduleVoiceStart(event);},{passive:false});btn.addEventListener('touchmove',handleVoiceTouchMove,{passive:true});btn.addEventListener('mouseup',cancelVoicePress);btn.addEventListener('mouseleave',function(){if(window.__voiceRecording||voicePressTimer)cancelVoicePress();});btn.addEventListener('touchend',function(event){event.preventDefault();cancelVoicePress();},{passive:false});btn.addEventListener('touchcancel',cancelVoicePress);}"
