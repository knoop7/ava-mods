...
public class WebConsoleServer {
    ...
    private String buildPage(boolean authed, String sessionId) {
        ...
        return "<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no,viewport-fit=cover'>"
            ...
            + "async function loadConfig(){try{const r=await fetch(apiUrl('/api/config'),{credentials:'include',headers:apiHeaders()});const j=await r.json();if(j.ok){document.getElementById('cfgProvider').value=j.provider||'openai';document.getElementById('cfgModel').value=j.model||'';document.getElementById('cfgApiUrl').value=j.custom_api_url||'';document.getElementById('cfgApiKey').value=j.api_key||'';document.getElementById('cfgMaxToolIterations').value=j.max_tool_iterations||'30';}}catch(e){console.error('loadConfig error',e);}}"
            + "let saveTimer=null;async function saveConfig(){if(saveTimer)clearTimeout(saveTimer);saveTimer=setTimeout(async()=>{try{const maxToolIterations=document.getElementById('cfgMaxToolIterations');let maxToolIterationsValue=maxToolIterations&&maxToolIterations.value?parseInt(maxToolIterations.value,10):30;if(!Number.isFinite(maxToolIterationsValue))maxToolIterationsValue=30;maxToolIterationsValue=Math.max(1,Math.min(100,maxToolIterationsValue));if(maxToolIterations)maxToolIterations.value=String(maxToolIterationsValue);const r=await fetch(apiUrl('/api/config'),{method:'POST',credentials:'include',headers:apiHeaders({'Content-Type':'application/json'}),body:JSON.stringify({provider:document.getElementById('cfgProvider').value,model:document.getElementById('cfgModel').value,custom_api_url:document.getElementById('cfgApiUrl').value,api_key:document.getElementById('cfgApiKey').value,max_tool_iterations:maxToolIterationsValue})});const j=await r.json();document.getElementById('configStatus').textContent=j.ok?'Saved':'Failed';}catch(e){document.getElementById('configStatus').textContent='Error';}},500);}function toggleApiKey(){const input=document.getElementById('cfgApiKey');if(!input)return;input.type=input.type==='password'?'text':'password';}"
