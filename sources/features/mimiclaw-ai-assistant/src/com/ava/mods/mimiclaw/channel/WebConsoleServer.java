...
public class WebConsoleServer {
    ...
    private String buildPage(boolean authed, String sessionId) {
        ...
        return "<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no,viewport-fit=cover'>"
            ...
            + "function renderSendButton(){const btn=document.getElementById('sendBtn');const input=document.getElementById('message');if(!btn)return;if(isVoiceUiLocked()){if(window.__voiceRecording||window.__voicePermissionPending){btn.innerHTML=svgMicIcon;btn.title=window.__voicePermissionPending?'Preparing microphone':'Listening';btn.classList.add('recording');btn.classList.remove('busy');if(input)input.placeholder=window.__voicePermissionPending?'Preparing microphone...':'Listening...';}return;}btn.innerHTML=window.__openclawBusy?svgStopIcon:svgSendIcon;btn.title=window.__openclawBusy?'Stop':'Send or hold to speak';btn.classList.remove('recording');btn.classList.toggle('busy',!!window.__openclawBusy);if(input)input.placeholder=window.__openclawBusy?'AI is processing...':'Type a message...';}"
            + "function setComposerBusy(busy){window.__openclawBusy=!!busy;renderSendButton();}"
            + "function syncBusyFromStatus(status){setComposerBusy(isBusyStatus(status));}"
            ...
            + "function setVoiceButtonState(mode){if(mode==='pending'){window.__voicePermissionPending=true;window.__voiceRecording=false;}else if(mode==='recording'){window.__voicePermissionPending=false;window.__voiceRecording=true;}else{window.__voicePermissionPending=false;window.__voiceRecording=false;}renderSendButton();}"
            + "function resetVoiceState(){window.__voiceRecording=false;window.__voicePressActive=false;window.__voicePermissionPending=false;window.__voiceLongPress=false;if(voicePressTimer){clearTimeout(voicePressTimer);voicePressTimer=null;}renderSendButton();}"
