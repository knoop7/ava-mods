...
public class WebConsoleServer {
    ...
    private String buildPage(boolean authed, String sessionId) {
        ...
        return "<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no,viewport-fit=cover'>"
            ...
            + ".statusPill{padding:0;background:transparent;font-size:0;display:flex;align-items:center;}.statusPill.st-ai .dot{background:#a855f7;}.statusPill.st-ok .dot{background:#22c55e;}.statusPill.st-warn .dot{background:#eab308;}.statusPill.st-err .dot{background:#ef4444;}.dot{width:10px;height:10px;border-radius:50%;background:var(--muted);display:inline-block;}.dot.spin{animation:pulse .8s ease-in-out infinite;}@keyframes pulse{0%,100%{opacity:1;}50%{opacity:.3;}}"
            ...
            + "function setChatStatus(status,opts){const el=document.getElementById('chatStatus');if(!el)return;const options=opts||{};if(options.plain){el.className='statusPill'+(options.className?' '+options.className:'');el.innerHTML='<i class=\"dot'+(options.className&&options.className.includes('st-')?' spin':'')+'\"></i>';return;}el.innerHTML=statusHtml(status);}"
            ...
            + "function statusHtml(s){const el=document.getElementById('chatStatus');if(!s){el.className='statusPill';return'<i class=\"dot\"></i>';}const p=String(s).split(': ');const c=p[0];const cls={idle:'',processing:'',thinking:'st-ai',llm_request:'st-ai',tool_use:'st-warn',responding:'st-ok',error:'st-err'};const stCls=cls[c]||(c.includes('error')?'st-err':'');el.className='statusPill'+(stCls?' '+stCls:'');const spin=(c==='processing'||c==='thinking'||c==='llm_request'||c==='tool_use')?' spin':'';return'<i class=\"dot'+spin+'\"></i>';}"
