...
public class WebConsoleServer {
    ...
    private String buildPage(boolean authed, String sessionId) {
        ...
        return "<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no,viewport-fit=cover'>"
            ...
            + "function renderWorkspace(status,detail){detail=detail||{};const agent=document.getElementById('wsAgent');const tokens=document.getElementById('wsTokens');const model=document.getElementById('wsModel');const history=document.getElementById('wsHistory');const err=document.getElementById('wsError');const statusEl=document.getElementById('workspaceStatus');if(agent)agent.textContent=workspaceValue(detail.agentStatus||status,'idle');if(tokens)tokens.textContent=workspaceValue(detail.totalTokens, '0');if(model)model.textContent=workspaceValue(detail.model,'-');if(history)history.textContent=workspaceValue(detail.historyCount,'0');if(err)err.textContent=workspaceValue(detail.webConsoleError||'', detail.agentStatus&&String(detail.agentStatus).indexOf('error')>=0 ? workspaceValue(detail.agentStatus,'Error') : 'None');if(statusEl)statusEl.textContent=status==='error'?'Error':(status||'');renderStatusJson(detail);}"
