...
public class WebConsoleServer {
    ...
    private String buildPage(boolean authed, String sessionId) {
        ...
        return "<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no,viewport-fit=cover'>"
            ...
            + ".workspaceJson{margin-top:0;max-height:180px;font-size:10px;line-height:1.5;padding:10px 11px;border-radius:12px;}"
            ...
            + "<div class='settingsCard'><div class='navTitle'>Workspace<span class='cardStatus' id='workspaceStatus'></span></div><div class='workspaceDebug'><div class='workspaceGrid'>"
            ...
            + "</div><div class='workspaceStat'><div class='workspaceStatLabel'>LLM</div><div class='workspaceStatValue' id='wsLlm'>-</div></div>"
            ...
            + "function renderWorkspace(status,detail){detail=detail||{};const agent=document.getElementById('wsAgent');const tokens=document.getElementById('wsTokens');const model=document.getElementById('wsModel');const history=document.getElementById('wsHistory');const llm=document.getElementById('wsLlm');const err=document.getElementById('wsError');const statusEl=document.getElementById('workspaceStatus');if(agent)agent.textContent=workspaceValue(detail.agentStatus||status,'idle');if(tokens)tokens.textContent=workspaceValue(detail.totalTokens, '0');if(model)model.textContent=workspaceValue(detail.model,'-');if(history)history.textContent=workspaceValue(detail.historyCount,'0');if(llm)llm.textContent=workspaceValue(detail.provider,'-')+' / '+workspaceValue(detail.apiUrl,'-');if(err)err.textContent=workspaceValue(detail.webConsoleError||'', detail.agentStatus&&String(detail.agentStatus).indexOf('error')>=0 ? workspaceValue(detail.agentStatus,'Error') : 'None');if(statusEl)statusEl.textContent=status==='error'?'Error':(status||'');renderStatusJson(detail);}"
