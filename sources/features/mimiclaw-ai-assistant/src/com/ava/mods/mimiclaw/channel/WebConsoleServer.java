...
public class WebConsoleServer {
    ...
    private String buildPage(boolean authed, String sessionId) {
        ...
        return "<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no,viewport-fit=cover'>"
            ...
            + "function isVoiceUiLocked(){return !!(window.__voiceRecording||window.__voicePressActive||window.__voicePermissionPending);}"
            + "function setComposerBusy(busy){const btn=document.getElementById('sendBtn');const input=document.getElementById('message');window.__openclawBusy=!!busy;if(btn&&!isVoiceUiLocked()){btn.innerHTML=busy?svgStopIcon:svgSendIcon;btn.title=busy?'Stop':'Send or hold to speak';btn.classList.toggle('busy',!!busy);btn.classList.remove('recording');}if(input&&!isVoiceUiLocked()){input.placeholder=busy?'AI is processing...':'Type a message...';}}"
            ...
            + "function sendOrStop(event){if(window.__suppressSendClick){window.__suppressSendClick=false;if(event){event.preventDefault();event.stopPropagation();}return;}if(window.__voiceRecording||window.__voicePressActive||window.__voicePermissionPending){if(event){event.preventDefault();event.stopPropagation();}return;}if(window.__openclawBusy){stopChat();}else{sendChat();}}"
            + "let voiceRecognition=null;let voicePressTimer=null;let voicePermissionReady=false;let voiceTouchStartX=0;let voiceTouchStartY=0;let voiceFinalTranscript='';const voiceTouchThreshold=12;window.__voiceRecording=false;window.__voicePressActive=false;window.__voicePermissionPending=false;window.__voiceLongPress=false;window.__suppressSendClick=false;"
            ...
            + "function ensureVoiceRecognition(){if(voiceRecognition)return voiceRecognition;const SpeechRecognition=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SpeechRecognition)return null;const recognition=new SpeechRecognition();recognition.lang=getSpeechLang();recognition.continuous=true;recognition.interimResults=true;recognition.maxAlternatives=1;try{if('requiresOnDeviceRecognition' in recognition)recognition.requiresOnDeviceRecognition=true;}catch(e){}try{if('processLocally' in recognition)recognition.processLocally=true;}catch(e){}recognition.onresult=function(event){const input=document.getElementById('message');let live=(voiceFinalTranscript||'').trim();for(let i=event.resultIndex;i<event.results.length;i++){const result=event.results[i];const text=((result&&result[0]&&result[0].transcript)||'').trim();if(!text)continue;if(result.isFinal){voiceFinalTranscript=(voiceFinalTranscript?voiceFinalTranscript+' ':'')+text;live=voiceFinalTranscript.trim();}else{live=((voiceFinalTranscript||'').trim()+' '+text).trim();}}if(input){input.value=live;input.dispatchEvent(new Event('input',{bubbles:true}));}};recognition.onerror=function(){finishVoiceRecording(false);};recognition.onend=function(){finishVoiceRecording(true);};voiceRecognition=recognition;return recognition;}"
            ...
            + "function resetVoiceState(){window.__voiceRecording=false;window.__voicePressActive=false;window.__voicePermissionPending=false;window.__voiceLongPress=false;if(voicePressTimer){clearTimeout(voicePressTimer);voicePressTimer=null;}setVoiceButtonState('idle');}"
            + "async function startButtonRecording(){if(window.__openclawBusy||window.__voiceRecording||!window.__voicePressActive||!window.__voiceLongPress)return;window.__voicePermissionPending=true;window.__suppressSendClick=true;setVoiceButtonState('pending');const granted=await ensureMicPermission();if(!granted||!window.__voicePressActive||!window.__voiceLongPress){window.__voicePermissionPending=false;resetVoiceState();if(!granted)document.getElementById('chatStatus').textContent='Mic denied';return;}const recognition=ensureVoiceRecognition();if(!recognition){window.__voicePermissionPending=false;resetVoiceState();document.getElementById('chatStatus').textContent='Voice unsupported';return;}voiceFinalTranscript='';window.__voicePermissionPending=false;window.__voiceRecording=true;setVoiceButtonState('recording');document.getElementById('chatStatus').textContent='Listening...';try{recognition.start();}catch(e){resetVoiceState();document.getElementById('chatStatus').textContent='Voice unavailable';}}"
            + "function stopButtonRecording(){if(voiceRecognition&&window.__voiceRecording){try{voiceRecognition.stop();}catch(e){finishVoiceRecording(false);}}else{resetVoiceState();}}"
            + "function finishVoiceRecording(fromEnd){const input=document.getElementById('message');const finalText=(input&&input.value?input.value.trim():'');const shouldSend=!!finalText&&!window.__openclawBusy&&!sending;resetVoiceState();if(finalText){document.getElementById('chatStatus').textContent=shouldSend?'Sending voice...':'Voice ready';}else{document.getElementById('chatStatus').textContent='Ready';}if(fromEnd&&shouldSend){setTimeout(()=>sendChat(),60);}setTimeout(()=>loadStatus(),220);}"
            + "function scheduleVoiceStart(event){if(window.__openclawBusy)return;if(event&&event.type==='mousedown'&&event.button!==0)return;window.__voicePressActive=true;window.__voiceLongPress=false;if(event&&event.touches&&event.touches[0]){voiceTouchStartX=event.touches[0].clientX;voiceTouchStartY=event.touches[0].clientY;}if(voicePressTimer)clearTimeout(voicePressTimer);voicePressTimer=setTimeout(()=>{voicePressTimer=null;if(window.__voicePressActive){window.__voiceLongPress=true;startButtonRecording();}},500);}"
            + "function releaseVoicePress(event){if(voicePressTimer){clearTimeout(voicePressTimer);voicePressTimer=null;}window.__voicePressActive=false;if(window.__voiceLongPress){window.__suppressSendClick=true;if(event&&event.preventDefault)event.preventDefault();stopButtonRecording();}}"
            + "function cancelVoicePress(){window.__voicePressActive=false;window.__voiceLongPress=false;if(voicePressTimer){clearTimeout(voicePressTimer);voicePressTimer=null;}if(window.__voiceRecording){stopButtonRecording();}else{resetVoiceState();}}"
            + "function handleVoiceTouchMove(event){if(!voicePressTimer||!event.touches||!event.touches[0])return;const dx=Math.abs(event.touches[0].clientX-voiceTouchStartX);const dy=Math.abs(event.touches[0].clientY-voiceTouchStartY);if(dx>voiceTouchThreshold||dy>voiceTouchThreshold){cancelVoicePress();}}"
            + "function setupVoiceInput(){const btn=document.getElementById('sendBtn');if(!btn)return;btn.addEventListener('mousedown',scheduleVoiceStart);btn.addEventListener('touchstart',function(event){event.preventDefault();scheduleVoiceStart(event);},{passive:false});btn.addEventListener('touchmove',handleVoiceTouchMove,{passive:true});btn.addEventListener('mouseup',releaseVoicePress);btn.addEventListener('mouseleave',function(){if(window.__voiceRecording||voicePressTimer||window.__voicePermissionPending)cancelVoicePress();});btn.addEventListener('touchend',function(event){event.preventDefault();releaseVoicePress(event);},{passive:false});btn.addEventListener('touchcancel',cancelVoicePress);}"
