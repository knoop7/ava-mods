...
public class WebConsoleServer {
    ...
    private String buildPage(boolean authed, String sessionId) {
        ...
        return "<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no,viewport-fit=cover'>"
            ...
            + ".composer button.attachBtn.recording{background:var(--accent);color:var(--accentText);box-shadow:0 0 0 4px var(--accentSoft);}"
            ...
            + "function parseStructuredContent(text){try{if(text==null)return null;if(Array.isArray(text))return text;if(typeof text==='object'){if(Array.isArray(text.content))return text.content;return null;}const raw=String(text);const trimmed=raw.trim();if(!trimmed||trimmed.length>500000)return null;if(!trimmed.includes('\"type\"'))return null;const jsonMatch=trimmed.match(/^\\s*(\\[\\s*\\{[\\s\\S]*\\}\\s*\\])\\s*$/);if(!jsonMatch)return null;try{const parsed=JSON.parse(jsonMatch[1]);if(Array.isArray(parsed)&&parsed.length>0&&parsed[0]&&parsed[0].type)return parsed;}catch(e){}return null;}catch(e){console.error('parseStructuredContent error',e);return null;}}"
            ...
            + "let voiceRecognition=null;let voicePermissionReady=false;let voiceFinalTranscript='';window.__voiceRecording=false;window.__sendingRequest=false;window.__stoppingRequest=false;let micLongPress=false;let micPressTimer=null;window.__voiceSendOnEnd=false;"
            ...
            + "function ensureVoiceRecognition(){if(voiceRecognition)return voiceRecognition;const SpeechRecognition=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SpeechRecognition)return null;const recognition=new SpeechRecognition();recognition.lang=getSpeechLang();recognition.continuous=false;recognition.interimResults=true;recognition.maxAlternatives=1;recognition.onresult=function(event){const input=document.getElementById('message');let live='';for(let i=0;i<event.results.length;i++){const result=event.results[i];const text=((result&&result[0]&&result[0].transcript)||'').trim();if(text){live+=(live?' ':'')+text;}}if(input&&live){input.value=live;input.dispatchEvent(new Event('input',{bubbles:true}));}};recognition.onerror=function(e){console.error('speech error',e.error);window.__voiceSendOnEnd=false;stopVoiceRecording();};recognition.onend=function(){if(window.__voiceRecording){window.__voiceRecording=false;renderMicButton();const shouldSend=window.__voiceSendOnEnd;window.__voiceSendOnEnd=false;if(shouldSend){const input=document.getElementById('message');if(input&&input.value.trim()){setTimeout(()=>sendChat(),50);}else{loadStatus();}}else{loadStatus();}}};voiceRecognition=recognition;return recognition;}"
            ...
            + "function stopVoiceRecording(sendAfter){if(voiceRecognition&&window.__voiceRecording){window.__voiceSendOnEnd=!!sendAfter;try{voiceRecognition.stop();}catch(e){}}window.__voiceRecording=false;renderMicButton();if(!sendAfter)loadStatus();}"
            ...
            + "function setupMicLongPress(){const btn=document.getElementById('micBtn');if(!btn)return;btn.addEventListener('mousedown',micPressStart);btn.addEventListener('touchstart',e=>{e.preventDefault();micPressStart(e);},{passive:false});btn.addEventListener('mouseup',micPressEnd);btn.addEventListener('mouseleave',micPressCancel);btn.addEventListener('touchend',e=>{e.preventDefault();micPressEnd(e);},{passive:false});btn.addEventListener('touchcancel',micPressCancel);}"
            + "function micPressStart(e){if(isSendButtonBusy()||window.__voiceRecording)return;micLongPress=false;if(micPressTimer)clearTimeout(micPressTimer);micPressTimer=setTimeout(()=>{micPressTimer=null;micLongPress=true;showToast('Release to send');startVoiceForLongPress();},400);}"
            + "function micPressEnd(e){if(micPressTimer){clearTimeout(micPressTimer);micPressTimer=null;toggleVoiceRecording();}else if(micLongPress&&window.__voiceRecording){stopVoiceRecording(true);}micLongPress=false;}"
            + "function micPressCancel(){if(micPressTimer){clearTimeout(micPressTimer);micPressTimer=null;}if(micLongPress&&window.__voiceRecording){stopVoiceRecording(false);}micLongPress=false;}"
