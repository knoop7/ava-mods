...
public class WebConsoleServer {
    ...
    private String buildPage(boolean authed, String sessionId) {
        ...
        return "<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no,viewport-fit=cover'>"
            ...
            + ".logo{position:relative;display:flex;align-items:flex-start;justify-content:space-between;}.logo>div:first-child{padding-left:10px;}.logo h1{margin:0;font-size:24px;line-height:1.15;}.logo p{margin:8px 0 0;color:var(--muted);line-height:1.5;font-size:13px;}"
            + ".logoInfo{cursor:pointer;min-width:0;}.logoInfo h1,.logoInfo p{transition:color .15s ease,opacity .15s ease;}.logoInfo:hover h1{color:var(--accent);}.logoInfo:hover p{opacity:.9;}"
            ...
            + ".logoPopover{position:absolute;left:10px;right:58px;top:calc(100% + 10px);z-index:30;display:none;background:var(--panel);border:1px solid var(--line);border-radius:16px;padding:14px;box-shadow:var(--shadow);}"
            + ".logoPopover.show{display:block;}"
            ...
            + "<div class='logo'><div class='logoInfo' onclick='toggleWorkspacePopover(event)'><h1>OpenClaw(Mini)</h1><p>Your own personal AI assistant. Any OS. Any Platform. The lobster way.</p></div>"
            ...
            + "<div class='logoPopover' id='workspacePopover'><div class='navTitle'>Workspace</div><div class='navItem'><strong>Web Console</strong><span>Port 18789, persistent local history</span></div><div class='jsonBox' id='statusJson'>Loading...</div></div>"
            + "</div>"
            ...
            + "function closeSidebar(){document.getElementById('sidebar').classList.remove('open');document.getElementById('overlay').classList.remove('open');closeWorkspacePopover();}"
            + "function toggleWorkspacePopover(event){if(event)event.stopPropagation();const pop=document.getElementById('workspacePopover');if(!pop)return;pop.classList.toggle('show');}"
            + "function closeWorkspacePopover(){const pop=document.getElementById('workspacePopover');if(pop)pop.classList.remove('show');}"
            ...
            + "document.addEventListener('click',function(event){const pop=document.getElementById('workspacePopover');const trigger=document.querySelector('.logoInfo');if(!pop||!trigger)return;if(pop.contains(event.target)||trigger.contains(event.target))return;pop.classList.remove('show');});"
