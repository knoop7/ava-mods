...
public class WebConsoleServer {
    ...
    private String buildPage(boolean authed, String sessionId) {
        ...
        return "<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no,viewport-fit=cover'>"
            ...
            + ".workspaceStatValue{font-size:12px;line-height:1.35;color:var(--text);font-weight:600;word-break:break-word;}"
            ...
            + "<div class='workspaceStat'><div class='workspaceStatLabel'>Agent</div><div class='workspaceStatValue' id='wsAgent'>-</div></div>"
            ...
            + "</div><div class='workspaceStat'><div class='workspaceStatLabel'>Error</div><div class='workspaceStatValue' id='wsError'>None</div></div>"
            + "<div class='jsonBox workspaceJson' id='statusJson'>Loading...</div></div></div>"
            + "</aside>"
            + "<main class='main'>"
            ...
            + "let showToolCalls=(function(){const v=localStorage.getItem('openclaw_showToolCalls');return v===null?true:v==='true';})();"
            ...
            + "function parseStructuredContent(text){if(text==null)return null;if(Array.isArray(text))return text;if(typeof text==='object'){if(Array.isArray(text.content))return text.content;return null;}const raw=String(text);const trimmed=raw.trim();if(!trimmed)return null;try{const parsed=JSON.parse(trimmed);if(Array.isArray(parsed))return parsed;if(parsed&&Array.isArray(parsed.content))return parsed.content;}catch(e){}if(!(trimmed.startsWith('[')&&trimmed.includes('\"type\"')))return null;try{const j=JSON.parse(trimmed);return Array.isArray(j)?j:null;}catch(e){return null;}}"
            ...
            + "function fileUrl(p){return p.startsWith('/')?'/api/file?path='+encodeURIComponent(p):p;}"
            ...
            + "function renderToolBlocks(blocks){let h='';(blocks||[]).forEach(t=>{if(t.type==='tool_use'){const id='tc_'+(++toolCallId);h+='<details class=\"toolCall\" id=\"'+id+'\"><summary onclick=\"event.stopPropagation()\">'+svgBolt+' '+escapeHtml(t.name||'tool')+'</summary><pre>'+escapeHtml(truncate(JSON.stringify(t.input||{},null,2),2000))+'</pre></details>';}if(t.type==='tool_result'){const id='tc_'+(++toolCallId);const c=typeof t.content==='string'?t.content:JSON.stringify(t.content,null,2);h+='<details class=\"toolCall\" id=\"'+id+'\"><summary onclick=\"event.stopPropagation()\">'+svgBolt+' Result</summary><pre>'+escapeHtml(truncate(c,2000))+'</pre></details>';}});return h;}"
            ...
            + "function renderWorkspace(status,detail){detail=detail||{};const agent=document.getElementById('wsAgent');const tokens=document.getElementById('wsTokens');const model=document.getElementById('wsModel');const history=document.getElementById('wsHistory');const err=document.getElementById('wsError');const statusEl=document.getElementById('workspaceStatus');const normalizedStatus=humanStatus(detail.agentStatus||status);if(agent)agent.textContent=workspaceValue(normalizedStatus,'idle');if(tokens)tokens.textContent=workspaceValue(detail.totalTokens, '0');if(model)model.textContent=workspaceValue(detail.model,'-');if(history)history.textContent=workspaceValue(detail.historyCount,'0');if(err)err.textContent=workspaceValue(detail.webConsoleError||'', normalizedStatus&&String(normalizedStatus).indexOf('error')>=0 ? workspaceValue(normalizedStatus,'Error') : 'None');if(statusEl)statusEl.textContent=normalizedStatus==='error'?'Error':workspaceValue(normalizedStatus,'idle');renderStatusJson(detail);}"
            + "function setComposerBusy(busy){const btn=document.getElementById('sendBtn');const input=document.getElementById('message');window.__openclawBusy=!!busy;if(btn&&!window.__voiceRecording){btn.innerHTML=busy?svgStopIcon:svgSendIcon;btn.title=busy?'Stop':'Send or hold to speak';btn.classList.toggle('busy',!!busy);}if(input&&!window.__voiceRecording){input.placeholder=busy?'AI is processing...':'Type a message...';}}"
            ...
            + "function sendOrStop(event){if(window.__suppressSendClick){window.__suppressSendClick=false;if(event){event.preventDefault();event.stopPropagation();}return;}if(window.__voiceRecording)return;if(window.__openclawBusy){stopChat();}else{sendChat();}}"
            + "let voiceRecognition=null;let voicePressTimer=null;let voicePointerActive=false;let voiceRecordingText='';let voicePermissionReady=false;let voicePressToken=0;let voiceTouchStartX=0;let voiceTouchStartY=0;let voiceLongPressTriggered=false;let voicePermissionPending=false;const voiceTouchThreshold=12;window.__voiceRecording=false;window.__suppressSendClick=false;"
            + "function getSpeechLang(){const lang=(navigator.language||'zh-CN').toLowerCase();if(lang.indexOf('zh')===0)return 'zh-CN';if(lang.indexOf('en')===0)return 'en-US';return navigator.language||'zh-CN';}"
            ...
            + "function ensureVoiceRecognition(){if(voiceRecognition)return voiceRecognition;const SpeechRecognition=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SpeechRecognition)return null;const recognition=new SpeechRecognition();recognition.lang=getSpeechLang();recognition.continuous=true;recognition.interimResults=true;recognition.maxAlternatives=1;try{if('requiresOnDeviceRecognition' in recognition)recognition.requiresOnDeviceRecognition=true;}catch(e){}try{if('processLocally' in recognition)recognition.processLocally=true;}catch(e){}recognition.onresult=function(event){let finalText='';let interimText='';for(let i=event.resultIndex;i<event.results.length;i++){const text=((event.results[i]&&event.results[i][0]&&event.results[i][0].transcript)||'').trim();if(!text)continue;if(event.results[i].isFinal)finalText+=(finalText?' ':'')+text;else interimText+=(interimText?' ':'')+text;}const value=(finalText||voiceRecordingText)+(interimText?((finalText||voiceRecordingText)?' ':'')+interimText:'');const input=document.getElementById('message');if(input&&value){input.value=value;input.dispatchEvent(new Event('input',{bubbles:true}));}if(finalText)voiceRecordingText=((voiceRecordingText?voiceRecordingText+' ':'')+finalText).trim();};recognition.onerror=function(){stopVoiceRecording(true);};recognition.onend=function(){if(window.__voiceRecording&&voicePointerActive){try{recognition.start();return;}catch(e){}}stopVoiceRecording(true);};voiceRecognition=recognition;return recognition;}"
            + "function setVoiceButtonState(mode){const btn=document.getElementById('sendBtn');const input=document.getElementById('message');if(!btn)return;if(mode==='recording'||mode==='pending'){btn.innerHTML=svgMicIcon;btn.title=mode==='pending'?'Preparing microphone':'Listening';btn.classList.add('recording');btn.classList.remove('busy');if(input)input.placeholder=mode==='pending'?'Preparing microphone...':'Listening...';return;}btn.innerHTML=window.__openclawBusy?svgStopIcon:svgSendIcon;btn.title=window.__openclawBusy?'Stop':'Send or hold to speak';btn.classList.remove('recording');btn.classList.toggle('busy',!!window.__openclawBusy);if(input)input.placeholder=window.__openclawBusy?'AI is processing...':'Type a message...';}"
            + "async function startVoiceRecording(token){if(window.__openclawBusy||window.__voiceRecording||token!==voicePressToken||!voiceLongPressTriggered)return;voicePermissionPending=true;setVoiceButtonState('pending');const granted=await ensureMicPermission();voicePermissionPending=false;if(!granted||token!==voicePressToken||!voiceLongPressTriggered){setVoiceButtonState('idle');if(!granted)document.getElementById('chatStatus').textContent='Mic denied';return;}const recognition=ensureVoiceRecognition();if(!recognition){setVoiceButtonState('idle');document.getElementById('chatStatus').textContent='Voice unsupported';return;}const input=document.getElementById('message');voiceRecordingText=(input&&input.value?input.value.trim():'');window.__voiceRecording=true;window.__suppressSendClick=true;setVoiceButtonState('recording');document.getElementById('chatStatus').textContent='Listening...';try{recognition.start();}catch(e){window.__voiceRecording=false;setVoiceButtonState('idle');document.getElementById('chatStatus').textContent='Voice unavailable';}}"
            + "function stopVoiceRecording(fromSystem){if(voicePressTimer){clearTimeout(voicePressTimer);voicePressTimer=null;}const wasRecording=window.__voiceRecording;window.__voiceRecording=false;voicePointerActive=false;voiceLongPressTriggered=false;voicePermissionPending=false;const input=document.getElementById('message');const shouldSend=!!(input&&input.value&&input.value.trim());if(voiceRecognition&&!fromSystem){try{voiceRecognition.stop();}catch(e){}}setVoiceButtonState('idle');if(wasRecording){document.getElementById('chatStatus').textContent=shouldSend?'Sending voice...':'Ready';if(shouldSend&&!window.__openclawBusy&&!sending){setTimeout(()=>sendChat(),80);}setTimeout(()=>loadStatus(),220);}}"
            + "function scheduleVoiceStart(event){if(window.__openclawBusy)return;if(event&&event.type==='mousedown'&&event.button!==0)return;voicePointerActive=true;voiceLongPressTriggered=false;voicePressToken++;if(event&&event.touches&&event.touches[0]){voiceTouchStartX=event.touches[0].clientX;voiceTouchStartY=event.touches[0].clientY;}if(voicePressTimer)clearTimeout(voicePressTimer);const token=voicePressToken;voicePressTimer=setTimeout(()=>{voicePressTimer=null;if(voicePointerActive&&token===voicePressToken){voiceLongPressTriggered=true;window.__suppressSendClick=true;startVoiceRecording(token);}},420);}"
            + "function cancelVoicePress(){voicePointerActive=false;voiceLongPressTriggered=false;voicePermissionPending=false;voicePressToken++;if(voicePressTimer){clearTimeout(voicePressTimer);voicePressTimer=null;}if(window.__voiceRecording)stopVoiceRecording(false);else setVoiceButtonState('idle');}"
            + "function releaseVoicePress(event){if(voicePressTimer){clearTimeout(voicePressTimer);voicePressTimer=null;}voicePointerActive=false;if(voiceLongPressTriggered){if(event&&event.preventDefault)event.preventDefault();if(window.__voiceRecording){stopVoiceRecording(false);}else if(voicePermissionPending){window.__suppressSendClick=true;}return;}}"
            + "function handleVoiceTouchMove(event){if(!voicePressTimer||!event.touches||!event.touches[0])return;const dx=Math.abs(event.touches[0].clientX-voiceTouchStartX);const dy=Math.abs(event.touches[0].clientY-voiceTouchStartY);if(dx>voiceTouchThreshold||dy>voiceTouchThreshold){clearTimeout(voicePressTimer);voicePressTimer=null;voicePointerActive=false;voicePressToken++;}}"
            + "function setupVoiceInput(){const btn=document.getElementById('sendBtn');if(!btn)return;btn.addEventListener('mousedown',scheduleVoiceStart);btn.addEventListener('touchstart',function(event){event.preventDefault();scheduleVoiceStart(event);},{passive:false});btn.addEventListener('touchmove',handleVoiceTouchMove,{passive:true});btn.addEventListener('mouseup',releaseVoicePress);btn.addEventListener('mouseleave',function(){if(window.__voiceRecording||voicePressTimer||voicePermissionPending)cancelVoicePress();});btn.addEventListener('touchend',function(event){event.preventDefault();releaseVoicePress(event);},{passive:false});btn.addEventListener('touchcancel',cancelVoicePress);}"
            ...
            + "async function stopChat(){if(!window.__openclawBusy)return;try{document.getElementById('chatStatus').textContent='Stopping...';queueStatusRefresh(60);const r=await fetch(apiUrl('/api/stop'),{method:'POST',credentials:'include',headers:apiHeaders({'Content-Type':'application/json'})});const j=await r.json();if(j&&j.ok){setComposerBusy(false);document.getElementById('chatStatus').textContent=j.status||'idle';addBubble('Generation stopped.','bot');queueStatusRefresh(120);}else{document.getElementById('chatStatus').textContent='Stop failed';queueStatusRefresh(120);}}catch(e){document.getElementById('chatStatus').textContent='Stop failed';queueStatusRefresh(120);console.error('stopChat error',e);}}"
            ...
            + "(async function init(){window.__openclawBusy=false;setComposerBusy(false);const cb=document.getElementById('showToolCalls');if(cb)cb.checked=showToolCalls;maybeShowInstallHint();setupDragDrop();setupVoiceInput();await loadConfig();await loadSkillControl();await reloadHistory();applyToolCallVisibility();setTimeout(connectSSE,100);})();"
            + "</script></body></html>";
    }
