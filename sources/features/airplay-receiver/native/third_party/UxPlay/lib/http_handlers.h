...
http_handler_play(raop_conn_t *conn, http_request_t *request, http_response_t *response,
    ...
    /* HLS master playlists go through FCUP; plain http(s) media URLs play directly
     * via on_video_play (android-airplay-server 0004-video-sender-compat). */
    ...
    } else if (!strncmp(playback_location, "http://", 7) ||
 ...
 play_error:;
    ...
    http_response_init(response, "HTTP/1.1", 400, "Bad Request");
    /* Do not force disconnect — keep AirPlay session for retry (compat patch). */
}
