...
public class WebConsoleServer {
    ...
    private String buildPage(boolean authed, String sessionId) {
        ...
        return "<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no,viewport-fit=cover'>"
            ...
            + "<details><summary>" + svgBolt + " Skills</summary><div class='settingsRow compact' id='skillControlList'></div><div class='status' id='skillStatus'></div></details>"
            + "<details><summary>" + svgHeart + " Timers</summary><div class='settingsRow compact' id='cronList'></div></details></div>"
            ...
            + "function renderCronList(cronJobs){const list=document.getElementById('cronList');if(!list)return;let h='';if(Array.isArray(cronJobs)&&cronJobs.length>0){h+='<div class=\"cronList\">';cronJobs.forEach(job=>{const name=job&&job.name?job.name:'Task';const meta=formatCronMeta(job);const checked=job&&job.enabled!==false;const id=job&&job.id?job.id:'';h+='<label class=\"skillItem cronItem\"><input type=\"checkbox\" data-job-id=\"'+escapeHtml(id)+'\" '+(checked?'checked':'')+' onchange=\"toggleCron(this)\"><div class=\"cronBody\"><div class=\"cronName\">'+escapeHtml(name)+'</div><div class=\"cronMeta\">'+escapeHtml(meta)+'</div></div></label>';});h+='</div>';}else{h='<div class=\"cronList\"><div class=\"cronItem\"><div class=\"cronBody\"><div class=\"cronName\">No scheduled tasks</div><div class=\"cronMeta\">AI can manage timers with the existing cron tools.</div></div></div></div>';}list.innerHTML=h;}"
