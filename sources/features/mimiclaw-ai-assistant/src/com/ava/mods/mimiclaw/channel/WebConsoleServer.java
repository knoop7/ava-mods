...
public class WebConsoleServer {
    ...
    private void handleClient(Socket socket) {
        try (Socket client = socket) {
            ...
            if ("GET".equals(method) && "/api/skill_config".equals(path)) {
                ...
                writeJson(output, 200, okJson().put("enabled", enabled).put("user_skills", userSkills), null);
                return;
            }
    ...
    private String buildPage(boolean authed, String sessionId) {
        ...
        return "<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no,viewport-fit=cover'>"
            ...
            + ".sidebar{width:320px;flex-shrink:0;background:var(--sidebar);border-right:1px solid var(--line);padding:calc(24px + env(safe-area-inset-top,0px)) 18px calc(24px + env(safe-area-inset-bottom,0px));display:flex;flex-direction:column;gap:18px;overflow-y:auto;}"
            ...
            + ".navCard,.settingsCard{background:var(--panel);border:1px solid var(--line);border-radius:16px;padding:14px;box-shadow:var(--shadow);}"
            ...
            + ".statusPill{padding:8px 14px;border-radius:999px;background:var(--accentSoft);color:var(--accent);font-size:12px;font-weight:700;display:flex;align-items:center;gap:6px;}.dot{width:8px;height:8px;border-radius:50%;background:var(--accent);display:inline-block;}.dot.spin{animation:pulse .8s ease-in-out infinite;}.dot.ok{background:#22c55e;}.dot.err{background:#ef4444;}@keyframes pulse{0%,100%{opacity:1;}50%{opacity:.3;}}"
            ...
            + ".codeCopyBtn svg{width:14px;height:14px;}"
            ...
            + ".toolCallGroup{display:flex;flex-direction:column;gap:4px;max-width:min(72ch,85%);align-self:flex-start;align-items:flex-start;}"
            + ".toolCall{margin:4px 0;border:1px solid var(--line);border-radius:12px;overflow:hidden;background:var(--field);max-width:100%;}"
            + ".bubble:has(.toolCall:only-child){background:transparent;padding:0;box-shadow:none;}"
            ...
            + ".inputWrap input{width:100%;padding-right:32px;position:relative;z-index:1;}"
            + ".eyeBtn{position:absolute;right:8px;top:50%;transform:translateY(-50%);width:18px;height:18px;padding:0;border:0;background:transparent;color:var(--muted);display:flex;align-items:center;justify-content:center;cursor:pointer;z-index:2;opacity:.85;}"
            ...
            + "<select id='cfgProvider' onchange='saveConfig()'><option value='openai'>OpenAI</option><option value='anthropic'>Anthropic</option></select>"
            + "<input id='cfgModel' placeholder='Model' onchange='saveConfig()'>"
            + "<input id='cfgApiUrl' placeholder='API URL' onchange='saveConfig()'>"
            + "<div class='inputWrap'><input id='cfgApiKey' type='password' placeholder='API Key' onchange='saveConfig()'><button class='eyeBtn' type='button' onclick='toggleApiKey()' aria-label='Toggle API key visibility'>" + svgEye + "</button></div>"
            + "<div class='status' id='configStatus'></div></div></details>"
            + "<details><summary>" + svgBolt + " Skills</summary><div class='settingsRow compact' id='skillControlList'></div><div class='status' id='skillStatus'></div></details></div>"
            + "<div class='settingsCard'><div class='navTitle'>Settings</div>"
            ...
            + "<details><summary>" + svgSettings + " Password</summary><div class='settingsRow compact'><input id='currentPassword' type='password' placeholder='Current'><input id='newPassword' type='password' placeholder='New'><button class='compactBtn' onclick='updatePassword()'>Update</button><div class='status' id='settingsStatus'></div></div></details></div>"
            + "</aside>"
            + "<main class='main'>"
            ...
            + "function applyToolCallVisibility(){const show=showToolCalls;document.querySelectorAll('.toolCall').forEach(el=>{el.style.display=show?'':'none';});document.querySelectorAll('.toolCallGroup').forEach(g=>{g.style.display=show?'':'none';});document.querySelectorAll('.bubble.bot').forEach(b=>{const tools=b.querySelectorAll('.toolCall');if(tools.length===0)return;let hasNonToolContent=false;for(const n of b.childNodes){if(n.nodeType===3&&n.textContent.trim()){hasNonToolContent=true;break;}if(n.nodeType===1&&!n.classList.contains('toolCall')){hasNonToolContent=true;break;}}if(!show&&!hasNonToolContent)b.style.display='none';else b.style.display='';});}"
            ...
            + "function renderUserContent(text){if(!text)return'';let html=text;html=html.replace(/<image_data>([^<]+)<\\/image_data>/g,(m,d)=>{let url=d.trim();if(!url.startsWith('data:'))url='data:'+url;return '<div class=\"userAttachments\"><div class=\"userAttachItem\"><img src=\"'+url+'\"/></div></div>';});html=html.replace(/\\[IMAGE:[^\\]]*\\]\\n?/g,'');html=html.replace(/\\[FILE:[^\\]]*\\]\\n?/g,'');html=html.replace(/```[\\s\\S]*?```/g,(m)=>'<pre>'+escapeHtml(m.slice(3,-3).trim())+'</pre>');return escapeHtml(html).replace(/\\n/g,'<br>').replace(/&lt;div class=&quot;userAttachments&quot;&gt;/g,'<div class=\"userAttachments\">').replace(/&lt;\\/div&gt;&lt;\\/div&gt;/g,'</div></div>').replace(/&lt;div class=&quot;userAttachItem&quot;&gt;/g,'<div class=\"userAttachItem\">').replace(/&lt;img src=&quot;([^&]+)&quot;\\/&gt;/g,'<img src=\"$1\"/>').replace(/&lt;pre&gt;/g,'<pre>').replace(/&lt;\\/pre&gt;/g,'</pre>');}"
            ...
            + "function appendToolGroupNode(html){if(!html)return;const container=document.createElement('div');container.className='toolCallGroup';container.innerHTML=html;document.getElementById('messages').appendChild(container);}"
            + "function addStructuredBubble(blocks,cls){if(!Array.isArray(blocks)||blocks.length===0)return false;let textParts=[];let toolParts=[];const flushText=()=>{if(textParts.length===0)return;appendBubbleNode(cls,renderPlainContent(textParts.join('\\n\\n')));textParts=[];};const flushTools=()=>{if(toolParts.length===0)return;appendToolGroupNode(renderToolBlocks(toolParts));toolParts=[];};blocks.forEach(t=>{if(t.type==='text'){const value=(t.text||'').trim();if(value&&value!=='null'){flushTools();textParts.push(value);}}else if(t.type==='tool_use'||t.type==='tool_result'){flushText();toolParts.push(t);}});flushText();flushTools();return true;}"
            + "function addBubble(text,cls,skipDup){const key=msgKey(cls,text||'');if(!skipDup&&shownMsgs.has(key))return;shownMsgs.add(key);const structured=parseStructuredContent(text);if(cls!=='user'&&structured){if(addStructuredBubble(structured,cls)){document.getElementById('messages').scrollTop=1e9;initViewer();applyToolCallVisibility();return;}}appendBubbleNode(cls,cls==='user'?(text||''):renderContent(text||''));document.getElementById('messages').scrollTop=1e9;initViewer();applyToolCallVisibility();}"
            + "function addBubbleHtml(html,cls){if(!html)return;const el=document.createElement('div');el.className='bubble '+cls;el.innerHTML=html;document.getElementById('messages').appendChild(el);document.getElementById('messages').scrollTop=1e9;initViewer();}"
            ...
            + "const svgCopy='<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"14\" height=\"14\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><rect x=\"2\" y=\"2\" width=\"13\" height=\"13\" rx=\"2\" ry=\"2\"/><path d=\"M9 5h8a2 2 0 0 1 2 2v8\"/></svg>';"
            ...
            + "function renderHistory(items){const box=document.getElementById('messages');box.innerHTML='';shownMsgs.clear();(items||[]).forEach(item=>addBubble(item.content||'',bubbleClass(item.role||'assistant')));box.scrollTop=1e9;}"
            + "function renderStatusJson(detail){document.getElementById('statusJson').textContent=JSON.stringify(detail||{},null,2);}"
            ...
            + "async function loadSkillControl(){const list=document.getElementById('skillControlList');if(!list)return;try{const r=await fetch(apiUrl('/api/skill_config'),{credentials:'include',headers:apiHeaders()});const j=await r.json();const enabled=j.ok?j.enabled||{}:{};const userSkills=j.user_skills||[];let h='<div class=\"skillTabs\"><span class=\"skillTab active\" data-tab=\"builtin\" onclick=\"toggleSkillTab(\\'builtin\\')\">Built-in</span>';if(userSkills.length>0){h+='<span class=\"skillTab\" data-tab=\"user\" onclick=\"toggleSkillTab(\\'user\\')\">User ('+userSkills.length+')</span>';}h+='</div><div id=\"skillPanel_builtin\" class=\"skillPanel\">';builtinSkills.forEach(s=>{const checked=(enabled[s.id]!==undefined)?enabled[s.id]:s.default;h+='<label class=\"skillItem\"><input type=\"checkbox\" data-skill=\"'+s.id+'\" '+(checked?'checked':'')+' onchange=\"toggleSkill(this)\"><span>'+s.name+'</span></label>'});h+='</div>';if(userSkills.length>0){h+='<div id=\"skillPanel_user\" class=\"skillPanel\" style=\"display:none\">';userSkills.forEach(s=>{const checked=(enabled[s.id]!==undefined)?enabled[s.id]:true;h+='<label class=\"skillItem\"><input type=\"checkbox\" data-skill=\"'+s.id+'\" '+(checked?'checked':'')+' onchange=\"toggleSkill(this)\"><span>'+s.name+'</span></label>'});h+='</div>';}list.innerHTML=h;}catch(e){console.error('loadSkillControl error',e);}}"
            + "async function toggleSkill(cb){const skillId=cb.dataset.skill;const enabled=cb.checked;try{const r=await fetch(apiUrl('/api/skill_config'),{method:'POST',credentials:'include',headers:apiHeaders({'Content-Type':'application/json'}),body:JSON.stringify({skill_id:skillId,enabled:enabled})});const j=await r.json();document.getElementById('skillStatus').textContent=j.ok?'Saved':'Failed';setTimeout(()=>{document.getElementById('skillStatus').textContent='';},1500);}catch(e){document.getElementById('skillStatus').textContent='Error';}}"
            + "window.addEventListener('resize',maybeShowInstallHint);window.addEventListener('orientationchange',maybeShowInstallHint);"
            + "(async function init(){window.__openclawBusy=false;setComposerBusy(false);const cb=document.getElementById('showToolCalls');if(cb)cb.checked=showToolCalls;maybeShowInstallHint();setupDragDrop();await loadConfig();await loadSkillControl();await reloadHistory();applyToolCallVisibility();setTimeout(connectSSE,100);})();"
            + "</script></body></html>";
    }
